...
HighLevelConsumer.prototype.connect = function () {
    ...
    if (this.client.ready) {
        console.log('connect - client ready');
        this.init();
    }

    this.client.on('ready', function () {
        console.log('connect - client ready event');
    ...
    function rebalance() {
        ...
        if (!self.rebalancing) {
            console.log('deregistered and rebalancing');
            deregister();
            ...
            operation.attempt(function (currentAttempt) {
                self.rebalanceAttempt(oldTopicPayloads, function (err) {
                    if (operation.retry(err)) {
                        console.log('nasty hack - retry ');
                        return;
                    }
                    if (err) {
                        self.rebalancing = false;
                        console.log('oh noes, i givez up');
                        ...
                        self.client.refreshMetadata(topicNames, function (err) {
                            console.log('trying to register', err);
                            register();
                            self.rebalancing = false;
                            if (err) {
                              self.emit('error', err);
    ...
    this.on('registered', function () {
        console.log('consumer ready and registered');
        rebalance();
    });
    ...
    async.series([
            ...
            function (callback) {
                ...
                self.client.zk.getConsumersPerTopic(self.options.groupId, function (err, obj) {
                    if (err) {
                        console.log('ZK assembly failed', self.id);
                        callback(err);
                    }
                    else {
            ...
            function (callback) {
                ...
                async.eachSeries(oldTopicPayloads, function (tp, cbb) {
                    ...
                    if (err) {
                        console.log('ZK released failed', self.id);
                        callback(err);
                    }
                    else {
                        callback();
            ...
            function (callback) {
                if (newTopicPayloads.length) {
                    ...
                    async.eachSeries(newTopicPayloads, function (tp, cbb) {
                        ...
                        if (err) {
                            console.log('ZK update failed',self.id);
                            callback(err);
                        }
                        else {
                            callback();
...
HighLevelConsumer.prototype.directCommit = function (commits, cb) {
